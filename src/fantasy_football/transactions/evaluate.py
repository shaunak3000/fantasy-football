...
def evaluate_move(
    ...
    """Simulate the season with and without a proposed add/drop.
    ...
    before = _title_odds(
        my_team_id, rosters, names, settings, schedule, playoff_teams, banked, trials, seed
    )
    before_mean, _ = roster_strength(rosters[my_team_id], settings)

    dropped = {getattr(p, "player", None) for p in drop}
    updated = dict(rosters)
    updated[my_team_id] = [
        p for p in rosters[my_team_id] if getattr(p, "player", None) not in dropped
    ] + list(add)

    after = _title_odds(
        my_team_id, updated, names, settings, schedule, playoff_teams, banked, trials, seed
    )
    ...
    if drop:
        labels.append("drop " + ", ".join(getattr(p, "player", "?") for p in drop))
