...
- [ ] 5. **Scoring engine** (`projections/scoring.py`) — convert any raw stat line into *this league's* points using the settings from step 2. **Gate:** recompute every 2025 box score from raw stats and match ESPN's published totals exactly. No projection is trusted until this passes.
- [ ] 6. **Ensemble projections** (`projections/`) — per-player mean *and variance*, weekly and rest-of-season.

      Note on sources, learned during step 3: `load_ff_rankings()` returns FantasyPros **expert consensus rankings** (ECR), not projected points. Only ESPN publishes an actual points projection. So the ensemble is not a simple average of three point estimates — it is ESPN's points projection, adjusted by where consensus rank disagrees with ESPN's implied rank, and cross-checked against an opportunity-based model built from `load_ff_opportunity()`. Converting rank to points requires a historical rank→points curve fit per position, which is extra work the original plan did not account for.

      The upside is that ECR ships `sd`, `best`, and `worst` — a direct measure of how much the experts disagree about each player. That is the only public dispersion signal available, and it feeds the variance estimates the risk-aware side of this repo depends on. Backtest 2024–2025 on rank correlation and on interval calibration; the variance estimate is load-bearing, so it gets validated as hard as the mean.
