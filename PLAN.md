...
- [x] 9. **Waivers + trades** (`transactions/`) — rest-of-season projections drive add/drop and trade evaluation, with every candidate move priced as a delta in P(first), not a delta in points. Waiver-priority aware (this league does not use FAAB).
      ...
      **Injury return weeks (fixed 2026-09-25).** Injured reserve used to count as out for the current week only, so A.J. Brown — out until November — was a healthy 15.9-point starter from week 4, and every trade touching the receiver room was mis-priced. ESPN's league API publishes a status and nothing else: no return date on any view, checked. So return weeks come from two sources (`data/injuries.py`), in order:
      ...
      1. **`data/injury_returns.json`**, maintained by hand from the news and tracked in git. A `return_date` resolves to the player's *own team's* first game on or after it — "back in November" is week 8 for New England (1 Nov @ MIA), and resolving by team handles Thursday games. It always wins, even over a healthy ESPN status, because the news runs ahead of the status. Delete an entry once the player is back.
      2. **Otherwise, for IR only, the NFL's four-game minimum** counted from the week the player was first seen on IR in `data/snapshots/`. A floor rather than a forecast — placement may predate the first capture, and many injuries run far longer — but it applies to every roster, so the other side of a trade is valued the same way.
